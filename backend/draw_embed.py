...
class DrawEmbed(QObject):
    ...
    def start(self, editor_id: str) -> bool:
        app = QApplication.instance()
        if app is None or not isinstance(app, QApplication):
            self._error = "DRAW_QT_WIDGETS_MISSING"
            return False
        spec = editor_by_id(editor_id)
        if spec is None:
            self._error = "DRAW_EDITOR_MISSING"
            return False
        path = str(spec["path"])
        tokens = tuple(str(token) for token in spec["tokens"])
        if self.attached() and self._editor == spec["id"]:
            self.show()
            return True
        self.stop()
        env = os.environ.copy()
        ...
        try:
            ...
            self._proc = None
            self._error = "DRAW_SPAWN_FAILED:" + type(exc).__name__
            return False
        self._editor = str(spec["id"])
        self._tokens = tokens
        self._attach_tries = 0
        self._attached = False
        self._error = ""
    ...
    def _try_attach(self) -> None:
        ...
        if proc.poll() is not None:
            self._proc = None
            self._error = "DRAW_EXITED"
            return
        ids = _window_xids(_descendants(int(proc.pid)), self._tokens)
        if ids:
        ...
        if self._attach_tries < 50:
            ...
            return
        self.stop()
        self._error = "DRAW_EMBED_TIMEOUT"
    ...
    def _embed_window(self, wid: int) -> bool:
        ...
        if foreign is None or handle is None:
            self.stop()
            self._error = "DRAW_FOREIGN_WINDOW_FAILED"
            return False
